...
static int32_t corridor_path_cmp(const void *key, const void *with) {
  return ((class corridor_path_t*) key)->cost - ((class corridor_path_t*) with)->cost;
}

// static int32_t corridor_path_cmp2(const void *key, const void *with) {
//   return ((corridor_path_t* )) key)->cost - ((corridor_path_t *) with)->cost;
// }

static void dijkstra_corridor(dungeon_t *d, pair_t from, pair_t to)
{
  static class corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  ...
  while ((p = static_cast<class corridor_path_t*>(heap_remove_min(&h)) )) {
    p->hn = NULL;

    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      for (x = to[dim_x], y = to[dim_y];
           ((signed)x != from[dim_x]) || ((signed)y != from[dim_y]);
...
static int connect_two_rooms(dungeon_t *d, class room_t *r1, class room_t *r2)
{
  pair_t e1, e2;
...
static int create_cycle(dungeon_t *d)
{
  ...
  for (i = max = 0; i < (signed)d->num_rooms - 1; i++) {
    for (j = i + 1; j < (signed)d->num_rooms; j++) {
...
static int smooth_hardness(dungeon_t *d)
{
  ...
  for (i = 1; i < 255; i += 20) {
    ...
    hardness[y][x] = i;
    if (i == 1) {
      head = tail = static_cast<queue_node_t*>(malloc(sizeof (*tail)));
    } else {
      tail->next = static_cast<queue_node_t*>(malloc(sizeof (*tail)));
      tail = tail->next;
    }
    tail->next = NULL;
    tail->x = x;
...
static int make_rooms(dungeon_t *d)
{
  ...
  for (i = MIN_ROOMS; i < MAX_ROOMS && rand_under(5, 8); i++)
    ;
  d->num_rooms = i;
  d->rooms = static_cast<class room_t*>(malloc(sizeof (*d->rooms) * d->num_rooms));
...
void render_dungeon(dungeon_t *d)
{
 ...
 void discover_map(dungeon_t *d)
 {
  pair_t p;
  int index_y = 0;
  int index_x = 0;
   for (p[dim_y] = d->pc.position[dim_y]-2; p[dim_y] <= d->pc.position[dim_y]+2; p[dim_y]++,index_y++) {
    for (p[dim_x] = d->pc.position[dim_x]-2; p[dim_x] <= d->pc.position[dim_x]+2; p[dim_x]++, index_x++) {
      if(d->pc.position[dim_y]-2 <0 || d->pc.position[dim_x]-2<0||p[dim_y]<0 ||p[dim_x]<0||p[dim_y] >DUNGEON_Y || p[dim_x]>  DUNGEON_X){
        continue;
      }
      ...
      if( p[dim_y] == d->pc.position[dim_y] &&  p[dim_x] == d->pc.position[dim_x] ){
	    d->discover_map[index_y][index_x].symbol = '@';
      d->discover_map[index_y][index_x].position[dim_y] = p[dim_y];
      d->discover_map[index_y][index_x].position[dim_x] = p[dim_x];
    }
      else{  
      switch (mapxy(p[dim_x], p[dim_y])) {
        case ter_wall:
        case ter_wall_immutable:
          d->discover_map[index_y][index_x].symbol = ' ';
          d->discover_map[index_y][index_x].position[dim_y] = p[dim_y]; 
...
int read_rooms(dungeon_t *d, FILE *f)
{
  ...
  for (i = 0; i < d->num_rooms; i++) {
    ...
    for (y = d->rooms[i].position[dim_y];
         (signed)y < d->rooms[i].position[dim_y] + d->rooms[i].size[dim_y];
         y++) {
      for (x = d->rooms[i].position[dim_x];
           (signed)x < d->rooms[i].position[dim_x] + d->rooms[i].size[dim_x];
           x++) {
