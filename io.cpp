...
{  
  ...
  const character_t *c1 = (character_t*)v1;
  const character_t *c2 = (character_t*) v2;

  return (dungeon->pc_distance[(*c1).position[dim_y]][(*c1).position[dim_x]] -
          dungeon->pc_distance[(*c2).position[dim_y]][(*c2).position[dim_x]]); // modified 

  // return (dungeon->pc_distance[(*c1)->position[dim_y]][(*c1)->position[dim_x]] -
  //         dungeon->pc_distance[(*c2)->position[dim_y]][(*c2)->position[dim_x]]); // original 
}
...
void io_display_teleport(dungeon_t *d) 
{
  uint32_t y, x;
  character_t *c;
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      if (d->character[y][x]) {
        ...
        if(d->character[y][x]->symbol == '*'||d->character[y][x]->symbol == '@'){
          d->character[y][x]->symbol = '@';
...
void io_display_with_fog(dungeon_t *d) 
{
  uint32_t y, x;
  //character_t *c;
  ...
  for (y = 0; y <5; y++) {
    for (x = 0; x <5; x++) {
        char sym = d->discover_map[y][x].symbol;
        //char sym = '.';
        int sym_y = d->discover_map[y][x].position[dim_y];
        int sym_x = d->discover_map[y][x].position[dim_x];
        mvaddch(sym_y+1, sym_x, sym);
      }
      refresh();
  }
